...
function initImageUpload(){
  ...
  document.getElementById('removeImageBtn')?.addEventListener('click', (e) => {
    e.stopPropagation();
    uploadedImage = null; aiResult = null;
    fileInput.value = '';
  ...
  document.getElementById('changeImageBtn')?.addEventListener('click', (e) => { e.stopPropagation(); fileInput.click(); });
}
...
function initAnalyze(){
  ...
  btn.addEventListener('click', () => {
    ...
    const detections = [
      { obj:'Garbage', sev:'High' }, { obj:'Pothole', sev:'Critical' },
      { obj:'Water Leak', sev:'Medium' }, { obj:'Broken Light Pole', sev:'Medium' }
    ];
    const pick = detections[Math.floor(Math.random()*detections.length)];
    const confidence = Math.floor(Math.random()*10) + 88; // 88-97
    const priority = pick.sev === 'Critical' || pick.sev === 'High' ? 'High' : 'Medium';
    ...
    const interval = setInterval(() => {
      ...
      if(progress >= 100){
        ...
        setTimeout(() => {
          ...
          aiResult = { detected: pick.obj, confidence, severity: pick.sev, priority };
          btn.disabled = false;
...
function initSubmit(){
  ...
  form.addEventListener('submit', async(e) => {
    ...
    let valid = true;
    if(!issue || !description){ showToast('Please fill in issue name and description', 'warn'); valid = false; }
    if(!selectedCategory){ showToast('Please select a category', 'warn'); valid = false; }
    if(!address || !area || !city){ showToast('Please complete the location fields', 'warn'); valid = false; }
    if(!valid) return;
  ...
  <div class="glass card text-center" style="max-width:420px;">
    ...
    <a href="reports.html" class="btn btn-primary btn-block" style="margin-top:12px;">View My Reports</a>
  </div>`;
    ...
    showToast('Failed to submit report. Make sure the backend is running.', 'warn');
  }
